...
void close_port(RehamoveDevice * r);
void pulse(RehamoveDevice * r, char * channel, int current, int pulse_width);
...
RehamoveDevice * open_port(const char * port_name) {
    ...
    if (f != NULL) {
        if (open_port_result == 0) {
            fprintf(f, "open_port(): ERROR! Unable to connect to port %s.\n", r->port_name);
        
        }
        else {
            fprintf(f, "open_port(): Successfully opened port.\n");
        }
        fclose(f);
    }
    r->device = device;
...
void close_port(RehamoveDevice * r) {
    ...
    if (f != NULL) {
        if (result == 0) {
            fprintf(f, "close_port(): ERROR! Unable to close port %s !\n", r->port_name);
        } 
        else {
            fprintf(f, "close_port(): Successfully closed port %s .\n", r->port_name);
        }
        fclose(f);
    }
    free(r);
...
void pulse(RehamoveDevice * r, char * channel, int current, int pulse_width) {
    ...
    if (strncmp(channel, "red", strlen("red")) == 0) {
        chosen_channel = Smpt_Channel_Red;
    }
    else if (strncmp(channel, "blue", strlen("blue")) == 0) {
        chosen_channel = Smpt_Channel_Blue;
    }
    else if (strncmp(channel, "black", strlen("gray")) == 0) {
...
int battery(RehamoveDevice * r) {
    ...
    */
    ...
    int battery_level = battery_ack.battery_level;
